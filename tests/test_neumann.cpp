...
TEST_CASE("Standard W(P) values are loaded correctly", "[standard_values]") {
  ...
  SECTION("Default confidence level (0.95)") {
    REQUIRE(standard_values.getWPValue(4) == Approx(0.7805));
    REQUIRE(standard_values.getWPValue(5) == Approx(0.8204));
    REQUIRE(standard_values.getWPValue(6) == Approx(0.8902));
    REQUIRE(standard_values.getWPValue(7) == Approx(0.9359));
  }

  SECTION("Custom confidence level (0.99)") {
    REQUIRE(standard_values.getWPValue(4, 0.99) == Approx(0.9027));
    REQUIRE(standard_values.getWPValue(5, 0.99) == Approx(0.9282));
  }
