...
[![Version](https://img.shields.io/badge/version-2.0.0-blue.svg)](https://github.com/Xyf0606/Neumann-Trend-Test/releases)
...
1. **下载预编译版本**
   ...
   - 下载最新的`NTT-x86_64-windows-v2.0.0.zip`文件
...
```
NeumannTrendTest-v2.0.0/
├── bin/                    # 可执行文件
│   ├── neumann_cli_app.exe # CLI应用
│   └── neumann_web_app.exe # Web应用
├── config/                 # 配置文件
│   ├── config.json         # 应用配置
│   └── translations.json   # 多语言翻译
├── data/sample/            # 示例数据
│   ├── sample_data.csv     # 基础示例
│   ├── pharmaceutical_data.csv # 药物数据
│   ├── large_dataset_45_points.csv # 大数据集测试
│   └── extra_large_dataset_55_points.csv # 超大数据集
├── ref/                    # 参考文件
...
├── web/                    # Web界面资源
├── start-cli-app.bat       # CLI启动脚本
├── start-web-app.bat       # Web启动脚本
└── README.md               # 使用说明
```
...
3. **命令行参数**
   ...
   ```bash
   # 直接处理文件
   ./bin/neumann_cli_app.exe -f data/sample/sample_data.csv
...
1. **启动 Web 服务器**
   ...
   ```bash
   ./bin/neumann_web_app.exe
   # 或指定端口
   ./bin/neumann_web_app.exe --port 8080
   ```
...
```json
{
  ...
  "defaultConfidenceLevel": 0.95, // 默认置信度: 0.95, 0.99, 0.999
...
**📧 联系方式**：如有问题或建议，请通过[GitHub Issues](https://github.com/Xyf0606/Neumann-Trend-Test/issues)联系我们。
