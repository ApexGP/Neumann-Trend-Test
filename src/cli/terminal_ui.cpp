...
void TerminalUI::initializeMenus() {
  ...
  mainMenu.items = {
      {"new_test", "运行新的诺依曼趋势测试", MenuItemType::ACTION, "",
       [this]() { runNeumannTest(); }},
      {"load_data", "加载数据集", MenuItemType::ACTION, "",
       [this]() { loadDataSet(); }},
      {"import_csv", "从CSV导入数据", MenuItemType::ACTION, "",
       [this]() { importFromCSV(); }},
...
std::vector<double> TerminalUI::promptForTimePoints(const std::string &prompt,
                                                    size_t count) {
  ...
  if (!response.empty() && std::tolower(response[0]) == 'y') {
    ...
    if (timePoints.size() != count) {
      std::cout << "警告: 时间点数量与数据点数量不匹配，将使用默认时间点。"
                << std::endl;
      timePoints.clear();
      for (size_t i = 0; i < count; ++i) {
        timePoints.push_back(static_cast<double>(i));
...
void TerminalUI::importFromCSV() {
  ...
  // 导入数据
  DataSet dataSet =
      DataManager::getInstance().importFromCSV(filePath, hasHeader);
...
void TerminalUI::runNeumannTest() {
  ...
  // 询问时间点
  std::vector<double> timePoints = promptForTimePoints(
      "是否使用默认时间点 (0, 1, 2, ...)?", dataPoints.size());
...
void TerminalUI::displayTestResults(const NeumannTestResults &results) {
  ...
  // 输出表头
  std::cout << std::left << std::setw(15) << "数据点" << std::setw(15)
            << "时间点" << std::setw(15) << "PG值" << std::setw(15)
            << "W(P)阈值" << std::setw(15) << "趋势判断" << std::endl;
  ...
  for (size_t i = 0; i < results.results.size(); ++i) {
    ...
    std::cout << std::left << std::setw(15) << results.data[dataIndex]
              << std::setw(15) << results.timePoints[dataIndex] << std::setw(15)
              << std::fixed << std::setprecision(4)
              << results.results[i].pgValue << std::setw(15)
  ...
  std::cout << "整体是否存在趋势: " << (results.overallTrend ? "是" : "否")
            << std::endl;
  std::cout << "最小PG值: " << std::fixed << std::setprecision(4)
            << results.minPG << std::endl;
...
void TerminalUI::showHelp() {
  ...
  std::cout
      << "诺依曼趋势测试是一种统计方法，用于评估数据集是否存在系统性趋势。"
      << std::endl;
  std::cout << "这种测试常用于药物稳定性研究和质量控制等领域。" << std::endl;
  std::cout << std::endl;

  std::cout << "使用指南:" << std::endl;
  std::cout << "1. 运行新的诺依曼趋势测试 - 直接输入数据并执行测试"
            << std::endl;
  ...
  std::cout << "- 标准W(P)值基于特定的统计表，用于判断趋势是否显著"
            << std::endl;
...
void TerminalUI::showAbout() {
  ...
  std::cout
      << "本工具用于执行诺依曼趋势测试，支持数据管理、测试执行和结果分析。"
      << std::endl;
  std::cout << "可广泛应用于药物稳定性研究、质量控制和时间序列分析等领域。"
            << std::endl;
