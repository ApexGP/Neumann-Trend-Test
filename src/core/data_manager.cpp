...
DataSet DataManager::importFromCSV(const std::string &filename,
                                   bool hasHeader) {
  DataSet dataSet;
...
bool DataManager::exportToCSV(const DataSet &dataSet,
  ...
  try {
    ...
    // 写入数据
    for (size_t i = 0;
         i < dataSet.dataPoints.size() && i < dataSet.timePoints.size(); ++i) {
      file << dataSet.timePoints[i] << "," << dataSet.dataPoints[i]
           << std::endl;
    }
