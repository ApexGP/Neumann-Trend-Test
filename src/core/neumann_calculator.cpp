...
NeumannCalculator::NeumannCalculator(double confidenceLevel)
    : confidenceLevel(confidenceLevel) {}

NeumannTestResults
NeumannCalculator::performTest(const std::vector<double> &data) {
  // 创建默认时间点 (0, 1, 2, ...)
...
NeumannCalculator::performTest(const std::vector<double> &data,
  ...
  for (size_t i = 3; i < data.size(); ++i) {
    ...
    // 获取对应样本数量的标准阈值
    double wpThreshold =
        StandardValues::getInstance().getWPValue(i + 1, confidenceLevel);
...
double NeumannCalculator::getConfidenceLevel() const { return confidenceLevel; }

double NeumannCalculator::calculatePG(const std::vector<double> &data,
                                      size_t endIndex) {
  // 确保不会超出范围
...
bool NeumannCalculator::determineTrend(double pgValue, int sampleSize) {
  // 获取对应的标准W(P)值
  double wpValue =
      StandardValues::getInstance().getWPValue(sampleSize, confidenceLevel);
