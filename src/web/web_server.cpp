...
void WebServer::start(bool background)
{
    ...
    std::string bindAddress = bindAddr ? bindAddr : "0.0.0.0";  // 默认绑定到所有地址

    // 启动服务器
    std::cout << "启动Web服务器，监听地址: " << bindAddress << ":" << port << std::endl;
...
void WebServer::registerApiEndpoints()
{
    ...
    // API端点: 获取数据集列表
    CROW_ROUTE(impl->app, "/api/datasets").methods(crow::HTTPMethod::GET)([this]() {
        return handleDataSetListRequest();
    });

    // API端点: 加载数据集
    CROW_ROUTE(impl->app, "/api/dataset")
        .methods(crow::HTTPMethod::GET)(
            [this](const crow::request &req) { return handleDataSetLoadRequest(req.body); });

    // API端点: 保存数据集
    CROW_ROUTE(impl->app, "/api/dataset")
        .methods(crow::HTTPMethod::POST)(
            [this](const crow::request &req) { return handleDataSetSaveRequest(req.body); });
}
...
std::string WebServer::handleNeumannTestRequest(const std::string &requestBody)
{
    try {
        ...
        if (dataPoints.size() < 4) {
            json error = {{"success", false}, {"error", "需要至少4个数据点才能进行诺依曼趋势测试"}};
            return error.dump();
        }
...
std::string WebServer::handleDataSetLoadRequest(const std::string &requestBody)
{
    try {
        // 解析请求体
        json request = json::parse(requestBody);
